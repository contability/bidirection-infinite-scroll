...
function useTopScreenEventHandler({
  ...
  const onWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    ...
    if (wheelPosition.current.move > wheelDistanceCriterion && isCalled.current) {
  ...
  const onTouchEnd = (event: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (touchStart - touchEnd < touchDistanceCriterion && scrollPosition.current.default < scrollOffset) {
      isCallable && callback();
